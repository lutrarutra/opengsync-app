...
class VisiumAnnotationForm(LibraryAnnotationWorkflow):
    ...
    def is_applicable(current_step: LibraryAnnotationWorkflow) -> bool:
        return bool(current_step.tables["library_table"]["library_type_id"].isin([t.id for t in LibraryType.get_visium_library_types()]).any())

    def __init__(self, seq_request: models.SeqRequest, uuid: str, formdata: dict | None = None):
        LibraryAnnotationWorkflow.__init__(self, seq_request=seq_request, step_name=VisiumAnnotationForm._step_name, formdata=formdata, uuid=uuid)

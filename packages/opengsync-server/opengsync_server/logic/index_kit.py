...
class IndexKitTable(HTMXTable):
    columns = [
        ...
        TableCol(title="Index Type", label="type", col_size=2, choices=cats.IndexType.as_selectable(), sortable=True, sort_by="type_id"),
...
def get_table_context(current_user: models.User, request: Request, **kwargs) -> dict:
    ...
    if (type_in := request.args.get("type_in")):
        ...
        try:
            type_in = [cats.IndexType.get(int(kit_type)) for kit_type in type_in]
            if type_in:

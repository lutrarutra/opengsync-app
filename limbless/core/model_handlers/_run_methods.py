...
def get_runs(
    ...
    query = self._session.query(models.Run).order_by(models.Run.id.desc())
...
def update_run(
    ...
    if r1_cycles is not None:
        run.r1_cycles = r1_cycles
    if r2_cycles is not None:
        run.r2_cycles = r2_cycles
    if i1_cycles is not None:
        run.i1_cycles = i1_cycles
    if i2_cycles is not None:
        run.i2_cycles = i2_cycles

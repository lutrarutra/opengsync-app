...
class BarcodeConstraintsForm(HTMXFlaskForm):
    ...
    def validate(self) -> bool:
        ...
        additional_i7: list[str] = []
        additional_i5: list[str] = []
        ...
        if self.kit.selected.data is not None:
            ...
            additional_i7 = barcodes_df["sequence_i7"].to_list()
            additional_i5 = barcodes_df["sequence_i5"].to_list()
            additional_sequences = [s1 + s2 for s1, s2 in zip(additional_i7, additional_i5)]
        ...
        if not sequences_i7 and not sequences_i5:
            if not additional_sequences:
                ...
                for a in needed_additions:
                    ...
                    for s in a:
                        ...
                        res.append(
                            DualIndex(
                                name_i7=mapping.at[seq_i7, "name_i7"],
                                name_i5=mapping.at[seq_i7, "name_i5"],
                                well=mapping.at[seq_i7, "well"],
                                sequence_i7=seq_i7,
                                sequence_i5=seq_i5
                            )
                        )
            ...
            if sequences_i7 and not utils.check_index_constraints(sequences_i7):
                if not additional_i7:
                    self.spreadsheet.add_general_error("i7 index constraints not met. Select kit another kit.")
                else:
                    needed_additions_i7 = utils.generate_valid_combinations(sequences_i7, additional_indices=additional_i7)
            ...
            if sequences_i5 and not utils.check_index_constraints(sequences_i5):
                if not additional_i5:
                    self.spreadsheet.add_general_error("i5 index constraints not met. Select kit another kit.")
                else:
                    needed_additions_i5 = utils.generate_valid_combinations(sequences_i5, additional_indices=additional_i5)

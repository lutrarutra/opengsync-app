...
class UploadLibraryPrepSpreadsheetAction(HTMXForm):
    template_path = "actions/upload-prep_table.html"

    spreadsheet = inputs.file.SpreadsheetFileField(
        columns=[
    ...
    def __init__(self, lab_prep: models.LabPrep) -> None:
        super().__init__()
        ...
        self.spreadsheet.configure(df=df, csrf_token=self.csrf_token_value)
        self.post_url = responses.url_for(f"{self.__class__.__name__}.Submit", lab_prep_id=lab_prep.id)

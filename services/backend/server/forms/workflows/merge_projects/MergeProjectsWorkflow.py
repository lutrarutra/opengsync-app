...
# from .. import MergeProjectsForm


class MergeProjectsWorkflow(HTMXWorkflow):
    ...
    def Previous(cls, step: str) -> WorkflowFunc:
        def dependency(
            workflow: "MergeProjectsWorkflow" = Depends(MergeProjectsWorkflow.Init(step)),
    ...
    def Router(cls) -> APIRouter:
        router = APIRouter(prefix="/merge-projects", tags=["merge-projects"], dependencies=[Depends(dependencies.require_insider)])
        router.add_api_route("/begin", MergeProjectsWorkflow.Begin(), methods=["GET"], name="MergeProjectsWorkflow.begin")
        return router

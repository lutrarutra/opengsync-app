...
# from .. import BarcodeConstraintsForm


class CheckBarcodeConstraintsWorkflow(HTMXWorkflow):
    ...
    def Previous(cls, step: str) -> WorkflowFunc:
        def dependency(
            workflow: "CheckBarcodeConstraintsWorkflow" = Depends(CheckBarcodeConstraintsWorkflow.Init(step)),
    ...
    def Router(cls) -> APIRouter:
        router = APIRouter(prefix="/check-barcode-constraints", tags=["check-barcode-constraints"], dependencies=[Depends(dependencies.require_insider)])
        router.add_api_route("/begin", CheckBarcodeConstraintsWorkflow.Begin(), methods=["GET"], name="CheckBarcodeConstraintsWorkflow.begin")
        return router

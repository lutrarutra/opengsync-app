...
# from .. import dilute_pools as wff


class DilutePoolsWorkflow(HTMXWorkflow):
    ...
    def Previous(cls, step: str) -> WorkflowFunc:
        def dependency(
            workflow: "DilutePoolsWorkflow" = Depends(DilutePoolsWorkflow.Init(step)),
        ) -> "DilutePoolsWorkflow":
    ...
    def Router(cls) -> APIRouter:
        router = APIRouter(prefix="/dilute-pools/{experiment_id}", tags=["dilute-pools"], dependencies=[Depends(dependencies.require_insider)])
        router.add_api_route("/begin", DilutePoolsWorkflow.Begin(), methods=["GET"], name="DilutePoolsWorkflow.begin")
        return router

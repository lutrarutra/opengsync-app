...
from .. import index_check as wf


class IndexCheckWorkflowStep(HTMXWorkflowStep):
    workflow: "IndexCheckWorkflow"
    ...
    def Init(cls: type[Self]) -> FormFunc:
        def dependency(
            workflow: IndexCheckWorkflow = Depends(IndexCheckWorkflow.Init(cls.__name__))
        ) -> Self:
    ...
    def Validate(cls: type[Self]) -> FormFunc:
        def dependency(
            form: Self = Depends(super(IndexCheckWorkflowStep, cls).Validate()),
        ) -> Self:
            return form
        return dependency


class IndexCheckWorkflow(HTMXWorkflow):
    ...
    def Init(cls, step: str) -> WorkflowFunc:
        def dependency(
            ...
            if not current_user.is_insider:
                raise exc.NoPermissionsException("You do not have permission to use this workflow.")
    ...
    def Begin(cls) -> RouteFunc:
        def route(
            form: wf.SelectLibrariesForm = Depends(wf.SelectLibrariesForm.Init()),
        ):
            return form.make_response()
        return route

    def get_next_step(self, form: "IndexCheckWorkflowStep") -> "IndexCheckWorkflowStep":
        """Advance to the next index check step and remember how to go back."""
        self.add_step(form.__class__.__name__)

        match form.__class__:
            case wf.SelectLibrariesForm:
                next_step_class = wf.VerifyOrientationsForm
            case wf.VerifyOrientationsForm:
                next_step_class = wf.CompleteIndexCheckForm
            case wf.CompleteIndexCheckForm:
                next_step_class = wf.CompleteIndexCheckForm
            case _:
                raise exc.OpeNGSyncServerException(
                    f"Unknown form class {form.__class__.__name__} in IndexCheckWorkflow."
                )

        next_step = next_step_class(workflow=self)
        # Store the back URL on the destination step.
    ...
    def Router(cls) -> APIRouter:
        router = APIRouter(prefix="/index-check", tags=["index-check"])
        router.add_api_route("/begin", IndexCheckWorkflow.Begin(), methods=["GET"], name="IndexCheckWorkflow.Begin")
        router.include_router(wf.SelectLibrariesForm.Router(cls.__name__))
        router.include_router(wf.VerifyOrientationsForm.Router(cls.__name__))
        router.include_router(wf.CompleteIndexCheckForm.Router(cls.__name__))
        return router
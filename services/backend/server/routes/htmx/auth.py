...
async def get_login_form(
    ...
    if current_user:
        return responses.htmx_response(redirect="dashboard")
...
async def login_user(
    response = Depends(auth_forms.LoginForm.process_request),
):
    return response
...
async def complete_registration(
    response = Depends(auth_forms.CompleteRegistrationForm.process_request),
):
    return response
...
def rgb_to_hex(red: float | tuple[float, float, float], green: float | None = None, blue: float | None = None) -> str:
    """Converts (0,1) based RGB values to a hex string 'rrggbb'"""
    if green is None or blue is None:
        assert isinstance(red, tuple)
        red, green, blue = red
    assert green is not None and blue is not None and isinstance(red, (int, float))
...
def rgb_to_ms_hls(red: float | str, green: float | None = None, blue: float | None = None) -> tuple[int, int, int]:
    ...
    if green is None:
        if isinstance(red, str):
            ...
            red, green, blue = red  # type: ignore
    assert green is not None and blue is not None and isinstance(red, (int, float))
    h, l, s = rgb_to_hls(red, green, blue)
    return (int(round(h * HLSMAX)), int(round(l * HLSMAX)), int(round(s * HLSMAX)))

def ms_hls_to_rgb(hue: int, lightness: int | None = None, saturation: int | None = None) -> tuple[float, float, float]:
    """Converts HLSMAX based HLS values to rgb values in the range (0,1)"""
    if lightness is None:
        hue, lightness, saturation = hue  # type: ignore
    assert lightness is not None and saturation is not None
...
def extract_style(theme_colors: list[str], cell: Cell | MergedCell) -> dict:
    ...
    if fill and fill.fill_type and fill.fill_type != "none":
        pattern_fill: PatternFill = fill  # type: ignore
        if pattern_fill.fgColor.type == "rgb":
            style["bg"] = { "rgb": f"#{argb_hex_to_rgb_hex(pattern_fill.fgColor.rgb)}" }
        elif pattern_fill.fgColor.type == "theme":
            style["bg"] = { "rgb": f"#{theme_and_tint_to_rgb(theme_colors, pattern_fill.fgColor.theme, pattern_fill.fgColor.tint)}" }
